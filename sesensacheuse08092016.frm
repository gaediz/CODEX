...
'rs.Open "SELECT     qaddb.pt_mstr.pt_desc1, coudb.zzsv_mstr.zzsv_coloris, qaddb.pt_mstr.pt_part From coudb.zzsv_mstr, qaddb.pt_mstr WHERE coudb.zzsv_mstr.zzsv_coloris = qaddb.pt_mstr.pt_part AND (coudb.zzsv_mstr.zzsv_lot = '1327176')", "FILE NAME=c:\connexion.udl"
'txtnomproduit(7).Text = rs!pt_desc1

'rs.Open "select formule,NOOF,CATMATPRE,qteutiletotale,couteau,UCONDIT,LIBMATPRE,dbnu42.d42ofno.codematpre from DBNU42.D42OFNO, DBNU42.D42MATPRE where DBNU42.D42MATPRE.codematpre = DBNU42.D42OFNO.codematpre AND (NOOF=" & "'" & noof & "'" & ")AND (CATMATPRE='02' Or CATMATPRE='04')AND (COUTEAU=" & "'" & couteau & "'" & ")", "FILE NAME=c:\connexion.udl"

'rs.Open "SELECT qaddb.pt_mstr.pt_desc1, qaddb.pt_mstr.pt_desc2,coudb.zzsv_mstr.zzsv_couteau, coudb.zzsv_mstr.zzsv_nbr,coudb.zzsv_mstr.zzsv_coloris, coudb.zzsv_mstr.zzsv_lot, coudb.zzsv_mstr.zzsv_part, coudb.zzsv_mstr.zzsv_qte_of, coudb.zzsv_mstr.zzsv_qte_pet, coudb.zzsvd_det.zzsvd_part, coudb.zzsvd_det.zzsvd_qte_of, coudb.zzsvd_det.zzsvd_qte_pet , coudb.zzsvd_det.zzsvd_poidemb, coudb.zzsvd_det.zzsvd_type From qaddb.pt_mstr, coudb.zzsvd_det, coudb.zzsv_mstr WHERE qaddb.pt_mstr.pt_part = coudb.zzsvd_det.zzsvd_part AND coudb.zzsvd_det.zzsvd_lot = coudb.zzsv_mstr.zzsv_lot AND (coudb.zzsv_mstr.zzsv_lot =" & "'" & Dialog.chaine1 & "'" & ") AND (coudb.zzsvd_det.zzsvd_type = 'pou')", "FILE NAME=c:\connexion.udl"
...
'Remplissage des champs en fonction de la requette...
        ...
        If Dialog1.doublage = False Then
            txtnumformule.Text = rs!zzsv_part
            nbrsac = (rs!zzsvd_qte_pet / rs!zzsvd_poidemb)
            entiere = Int(nbrsac)
            decimale = nbrsac - entiere
            complement = (decimale * rs!zzsvd_poidemb) '/ 100
        
          'text1(i).Text = rs!formule
            txtnomproduit(i).Text = rs!pt_desc1
            txtcodematpre(i).Text = rs!zzsvd_part
            txtqteutiletotale(i).Text = Round(rs!zzsvd_qte_pet, 1)
            txtcomplementrecette(i).Text = Round(complement, 1)
            txtnbrsac(i).Text = entiere
            
        ElseIf Dialog1.doublage = True Then
            txtnumformule.Text = rs!zzsv_part
            nbrsac = ((rs!zzsvd_qte_pet * 2) / rs!zzsvd_poidemb)
            entiere = Int(nbrsac)
            decimale = nbrsac - entiere
            complement = ((decimale * rs!zzsvd_poidemb)) '/ 100)
          ...
          'text1(i).Text = rs!formule
            txtnomproduit(i).Text = rs!pt_desc1
            txtcodematpre(i).Text = rs!zzsvd_part
            txtqteutiletotale(i).Text = Round((rs!zzsvd_qte_pet * 2), 1) '/ 100) * 2
            txtcomplementrecette(i).Text = Round(complement, 1)
            txtnbrsac(i).Text = entiere
        End If
